...
class WorkflowController < ApplicationController
  ...
  before_filter :login_required, :except => [:list, :index, :show, :download, :results, :gimme]

  layout  'application', :except => [:textify, :live_search]

  auto_complete_for :tag, :name

  before_filter :only => [:show, :rate, :comment, :tag, :bookmark, :download, :textify, :gimme] do |w|
    w.find_workflow("r")
  end
  ...
  def find_workflow(permission)
    if params[:id]
      begin
        if (workflow = Workflow.find(params[:id])).authorized? current_user, permission
          @workflow = workflow
        else
  ...
  def search
    unless params[:query].blank?
      ...
      @collection = Workflow.ferret_find(@query, {:page => {:size => 18, :current => params[:page], :first => 1}, :sort => s})
      render :action => 'results'
    else
      @collection = Workflow.find(:all, :page => {:size => 18, :current => params[:page], :first => 1})
      render :action => 'results'
    end
  end
  ...
  def update_sharing
    ...
    if @workflow.update_attribute :acl_r, total and @workflow.update_attribute :license, params[:license]
      flash[:notice] = "Sharing for #{params[:id]} Workflow was successfully updated."
  ...
  def create_shared_user
    ...
    if @shared_user.save
      flash[:notice] = "Shared User for #{params[:id]} successfully created!"
  ...
  def destroy_shared_user
    ...
    flash[:notice] = "Shared User #{params[:id]} successfully removed!"
    redirect_to :action => :sharing, :id => @workflow.id
  end
  ...
  def create_shared_project
    ...
    if @shared_project.save
      flash[:notice] = "Shared Project for #{params[:id]} successfully created!"
      redirect_to :action => :sharing, :id => @workflow.id
    else
      render :action => :new
    end
  ...
  def destroy_shared_project
    ...
    flash[:notice] = "Shared Project #{params[:id]} successfully removed!"
    redirect_to :action => :sharing, :id => @workflow.id
  end
