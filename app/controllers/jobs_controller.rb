...
class JobsController < ApplicationController
  ...
  def create
    ...
    remote_workflow = get_remote_workflow(TAVERNA_SERVICE_USER, workflow_id)
    
    response = submit_job(TAVERNA_SERVICE_USER, remote_workflow)      
  ...
  def get_sources(workflow)
    ...
    model  = parser.parse(workflow.scufl)
  ...
  def submit_workflow(user_url, workflow_id)
    ...
    req.basic_auth TAVERNA_SERVICE_USER_NAME, TAVERNA_SERVICE_PASSWORD
    req['Content-Type'] = 'application/vnd.taverna.scufl+xml'
    req.body = Workflow.find(workflow_id).scufl
