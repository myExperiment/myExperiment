...
class FlexImageModelTest < Test::Unit::TestCase
  ...
  def test_image_should_create_from_url
    img = Image.create!(:data => 'http://www.google.com/intl/en_ALL/images/logo.gif')
    assert(!img.new_record?, "Image was not properly created from url")
  end
  ...
  def test_resize_should_stretch_image
    @image.resize! :size => '50x50', :stretch => true
    assert_size('50x50', @image)
  end
  ...
  # Other Tests
  def test_different_class_attributes_with_different_classes_should_be_different
    assert_equal :jpg, Image.storage_format
    assert_equal :png, Image2.storage_format
  end
