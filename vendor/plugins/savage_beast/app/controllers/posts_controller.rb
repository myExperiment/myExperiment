class PostsController < ApplicationController
  ...
  def new
    ...
    if params[:reply_id]
      found = Post.find_by_id_and_topic_id_and_forum_id(params[:reply_id], @topic.id, @topic.forum) || raise(ActiveRecord::RecordNotFound)
      
      # append original post to reply
      @post.body = "bq. #{found.body}"
    end
  end
  ...
  protected
    ...
    def authorized?(forum=@forum, user=current_user)
      ...
      user.admin? or forum.public or Membership.find_by_project_id_and_user_id(forum.owner_id, user.id)
    end
    ...
    def find_forum_and_topic_and_post
      ...
      if @forum
        ...
        if params[:topic_id]
          ...
          flash[:notice] = "Error! No Topic ID supplied"
            redirect_to :controller => "forums", :action => "index" and return
        end
      else
      ...
      @post = Post.find_by_id_and_topic_id_and_forum_id(params[:id], @topic.id, @forum.id) || raise(ActiveRecord::RecordNotFound)
    end
